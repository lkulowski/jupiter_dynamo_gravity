...
To use Ferraro’s law, we need to know the magnetic field lines in the dynamo region and the angular velocity at the dynamo surface. We assume two forms for the magnetic field. First, we consider a magnetic field where the spectral poloidal scalar has a polynomial form. Second, we consider a magnetic field that minimizes Ohmic dissiation. At the dynamo surface, we assume that the zonal flow is proportional to the zonally averaged, non-dipolar theta-component of Jupiter's magnetic field. We allow for some small adjustments to this flow so that we can satisfy Ferraro's law. 

The plots below show how to construct dynamo zonal flow profiles using Ferraro's law. We extend the angular velocity associated with the zonal flow at the dynamo surface (red and blue curves in (b, e)) into the interior along magnetic field lines (a, d) to obtain resulting dynamo zonal flow profiles (c, f). The dynamo zonal flows shown here have RMS velocities of 10 cm/s.
